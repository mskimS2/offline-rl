...
def get_d4rl_mujoco_env_config(env: str, challenge: str):
    env = env.lower().strip()
    challenge = challenge.lower().strip()
    
    challenges = {"medium", "medium-replay", "medium-expert"}
    if challenge not in challenges:
        raise ValueError(f"Invalid dataset '{challenge}'.\nMust be one of {sorted(challenges)}.")

    env_map = {
        "walker2d": ("Walker2d-v3", 5000, f"walker2d-{challenge}-v2"),
        "halfcheetah": ("HalfCheetah-v3", 6000, f"halfcheetah-{challenge}-v2"),
        "hopper": ("Hopper-v3", 3600, f"hopper-{challenge}-v2"),
    }
    if env not in env_map:
        raise NotImplementedError(f"Unsupported env: {env}. Supported: {sorted(env_map.keys())}")
...
class D4RLTrajectoryDataset(Dataset):
    ...
    def __init__(self, dataset_path: str, context_len: int, rtg_scale: float) -> None:
        ...
        for traj in self.trajectories:
            ...
            traj['returns_to_go'] = discount_cumsum(traj['rewards'], 1.0).astype(np.float32) / rtg_scale
